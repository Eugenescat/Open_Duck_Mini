...
class BDXEnv(MujocoEnv, utils.EzPickle):
    ...
    def forward_gate(self):
        # Gate forward rewards to discourage earning progress via unstable leaning.
        h = float(self.data.body("base").xpos[2])
        roll, pitch = self.base_rpy()
        upright = self.upright_reward()
        single_support = self.support_side() != 0
    ...
    def step(self, a):
        ...
        if self.startup_cooldown > 0:
            ...
            reward = (
                0.28 * gate * self.forward_velocity_reward()
                + 0.24 * gate * self.forward_progress_reward()
                + 0.10 * self.forward_displacement_reward()
                + 0.12 * self.swing_foot_forward_reward()
                + 0.10 * self.contact_switch_reward()
                + 0.08 * self.single_support_reward()
                + 0.06 * gate * self.low_forward_speed_penalty()
                + 0.08 * self.height_reward()
                + 0.08 * self.upright_reward()
                + 0.04 * self.lateral_stability_reward()
                + 0.03 * self.yaw_stability_reward()
                + 0.03 * self.torque_reward()
                + 0.03 * self.support_flying_reward()
                + 0.03 * self.double_support_penalty()
                + 0.07 * self.imitation_pose_reward()
                + 0.01 * self.imitation_action_reward(a)
                + 0.09 * self.pitch_penalty()
                + 0.06 * self.roll_penalty()
            )
