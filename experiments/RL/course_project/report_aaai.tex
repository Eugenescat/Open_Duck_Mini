...
\paragraph{Extending the ablation to all four algorithms.} To test whether the walk-reward improvement generalizes beyond PPO, we additionally trained SAC, A2C, and TQC for $300{,}000$ steps (single seed) on the walk variant (Table~\ref{tab:walk_all}, Figure~\ref{fig:walk_all_curve}). The result reinforces the main finding of this project: the two on-policy methods (PPO and A2C) both learn to \emph{move forward}, while the two off-policy methods (SAC and TQC) instead drift \emph{backward} ($-0.035$ and $-0.022$~m/s respectively)---they learn some locomotion in the wrong direction, presumably because their replay buffers accumulate short backward-slip transitions that happen to avoid immediate termination. TQC achieves the highest return ($28.7$) despite moving backward, which once more illustrates that in this task ``return'' is a weak proxy for ``walking''. A2C's $0.096$~m/s at only $300$k steps is within a factor of $1.2$ of PPO's $0.116$~m/s at $1.5$M, confirming that on-policy methods are the correct fit for this task regime.

\begin{table}[t]
\centering
\small
\caption{All four algorithms on the walk variant (300k, single seed). On-policy methods move forward; off-policy methods drift backward.}
\label{tab:walk_all}
\begin{tabular}{lccc}
\toprule
Algo & Return & Ep.~Length & Fwd.~Speed (m/s) \\
\midrule
A2C & $10.1 \pm 0.0$ & $195.0 \pm 0.0$ & $\mathbf{+0.096 \pm 0.0}$ \\
SAC & $4.0 \pm 0.0$ & $169.0 \pm 0.0$ & $-0.035 \pm 0.003$ \\
TQC & $\mathbf{28.7 \pm 1.7}$ & $264.3 \pm 8.1$ & $-0.022 \pm 0.0$ \\
\midrule
\emph{PPO (1.5M, ref.)} & \emph{83.5 $\pm$ 13.0} & \emph{326.6 $\pm$ 31.2} & \emph{$\mathbf{+0.116 \pm 0.006}$} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{runs/walk_all_algos_300k/learning_curves_return.png}
\caption{Training return on the walk variant for the three non-PPO algorithms (single seed, 300k steps each). TQC achieves the highest return but its policy walks \emph{backward}; A2C's return is modest but it actually moves forward at $0.096$~m/s (see Table~\ref{tab:walk_all}).}
\label{fig:walk_all_curve}
\end{figure}
